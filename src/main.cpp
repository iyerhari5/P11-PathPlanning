        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i = 0; i < sensor_fusion.size(); i++)
			{
				...
				{
					//the other car is on the left of us
					car_left |= abs(check_car_s - car_s) < SAFE_DISTANCE;
				}
				else if (lane - other_car_lane == -1)
				{
					//the other car is on the right of us
					car_right |= abs(check_car_s - car_s) < SAFE_DISTANCE;
				}
			}
			...
			if (car_ahead)
			{
				...
				{
					//stay in the same lane but reduce velocity
					cout << " Car ahead. Number of points:" << prev_size << endl;
					ref_velocity -= 0.224;
					ref_velocity  = max(ref_velocity,0.224);
					car_ahead = false;
				}
			}
			else
